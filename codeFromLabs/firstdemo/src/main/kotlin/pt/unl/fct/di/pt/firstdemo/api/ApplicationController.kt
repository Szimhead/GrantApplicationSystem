...
class ApplicationController(val apps: ApplicationService, val reviewers: ReviewerService, val dataItems: DataItemService): ApplicationAPI {
    ...
    @PreAuthorize("hasRole({'ADMIN'}) or " +
            "(@SecurityService.canGetApplication(principal,#id) and (hasRole({'REVIEWER'}) or hasRole({'STUDENT'}) or hasRole({'CHAIR'})))")
    override fun getOne(id:Long) = ApplicationDTO(apps.getOne(id))

    @PreAuthorize("hasRole({'ADMIN'}) or " +
            "(@SecurityService.canEditApplication(principal,#id) and hasRole({'STUDENT'}))")
    override fun editApplication(id:Long, app: ApplicationDTO) = apps.editApplication(apps.getOne(id), ApplicationDAO(app))

    @PreAuthorize("hasRole({'ADMIN'}) or " +
            "(@SecurityService.canGetReviewsFromApplication(principal,#id) and hasRole({'CHAIR'}))")
    override fun getAllReviewsFromApplication(id:Long) = apps.getAllReviewsFromApplication(apps.getOne(id)).map { ReviewDTO(it) }

    @PreAuthorize("hasRole({'ADMIN'}) or " +
            "(@SecurityService.canGetReview(principal,#reviewId) and (hasRole({'REVIEWER'}) or hasRole({'STUDENT'}) or hasRole({'CHAIR'})))")
    override fun getOneReview(id:Long, reviewId:Long) = ReviewDTO(apps.getOneReview(apps.getOne(id), reviewId))

    @PreAuthorize("hasRole({'ADMIN'}) or " +
            "(@SecurityService.canDeleteReview(principal,#reviewId) and (hasRole({'REVIEWER'}) or hasRole({'CHAIR'})))")
    ...
    @PreAuthorize("hasRole({'ADMIN'}) or " +
            "(@SecurityService.canEditReview(principal,#review.id) and (hasRole({'REVIEWER'}) or hasRole({'CHAIR'})))")
    ...
    @PreAuthorize("hasRole({'ADMIN'}) or " +
            "(@SecurityService.canAddReview(principal,#review.id) and (hasRole({'REVIEWER'}) or hasRole({'CHAIR'})))")
